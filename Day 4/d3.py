...
class ThemePark:
    ...
    dict_of_games = {"Game1": [35.5, 5], "Game2": [40.0, 6], "Game3": [120.0, 10], "Game4": [60.0, 7],
                     "Game5": [25.0, 4]}
...
class Ticket:
    ...
    def calculate_amount(self, list_of_games):
        res = all(game in ThemePark.dict_of_games.keys() for game in list_of_games)
        if res:
            self.__ticket_amount = sum([ThemePark.get_amount(game) for game in list_of_games])
            return True
        else:
            return False
        # Remove pass and write the logic here
...
class Customer:
    ...
    def play_game(self):
        self.__points_earned = sum([ThemePark.get_points(game) for game in self.__list_of_games])
        if "Game3" in self.__list_of_games:
            self.__list_of_games += ["Game2"]
            self.__points_earned += ThemePark.dict_of_games["Game2"][1]
    ...
    def book_ticket(self):
        if self.__ticket.calculate_amount(self.__list_of_games):
            # print(self.__ticket.get_ticket_amount())
...
'''Represent customers and display all details of the customer, if he is able to book the ticket and play the game. 
Else, display appropriate error message '''
c1 = Customer("Shreyans", ["Game1", "Game3"])
c1.book_ticket()
